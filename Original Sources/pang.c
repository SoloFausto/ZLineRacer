        ...
        if (!pause)
        {
            ...
            for (int i = 0; i < MAX_BIG_BALLS; i++)
            {
                if (bigBalls[i].active)
                {
                    ...
                    if (((bigBalls[i].position.x + bigBalls[i].radius) >= screenWidth) || ((bigBalls[i].position.x - bigBalls[i].radius) <= 0))
                        bigBalls[i].speed.x *= -1;
                    if ((bigBalls[i].position.y - bigBalls[i].radius) <= 0)
                        bigBalls[i].speed.y *= -1.5;
            ...
            for (int i = 0; i < MAX_BIG_BALLS * 2; i++)
            {
                if (mediumBalls[i].active)
                {
                    ...
                    // Meteor vs wall collision logic
                    if (mediumBalls[i].position.x + mediumBalls[i].radius >= screenWidth || mediumBalls[i].position.x - mediumBalls[i].radius <= 0)
                        mediumBalls[i].speed.x *= -1;
                    if (mediumBalls[i].position.y - mediumBalls[i].radius <= 0)
            ...
            for (int i = 0; i < MAX_BIG_BALLS * 4; i++)
            {
                if (smallBalls[i].active)
                {
                    ...
                    // Meteor vs wall collision logic
                    if (smallBalls[i].position.x + smallBalls[i].radius >= screenWidth || smallBalls[i].position.x - smallBalls[i].radius <= 0)
                        smallBalls[i].speed.x *= -1;
                    if (smallBalls[i].position.y - smallBalls[i].radius <= 0)
            ...
            for (int i = 0; i < PLAYER_MAX_SHOOTS; i++)
            {
                if ((shoot[i].active))
                {
                    ...
                    {
                        if (bigBalls[a].active && (bigBalls[a].position.x - bigBalls[a].radius <= linePosition.x && bigBalls[a].position.x + bigBalls[a].radius >= linePosition.x) && (bigBalls[a].position.y + bigBalls[a].radius >= shoot[i].position.y))
                        {
                            shoot[i].active = false;
                            shoot[i].lifeSpawn = 0;
                ...
                if ((shoot[i].active))
                {
                    ...
                    {
                        if (mediumBalls[b].active && (mediumBalls[b].position.x - mediumBalls[b].radius <= linePosition.x && mediumBalls[b].position.x + mediumBalls[b].radius >= linePosition.x) && (mediumBalls[b].position.y + mediumBalls[b].radius >= shoot[i].position.y))
                        {
                            shoot[i].active = false;
                            shoot[i].lifeSpawn = 0;
                ...
                if ((shoot[i].active))
                {
                    ...
                    {
                        if (smallBalls[c].active && (smallBalls[c].position.x - smallBalls[c].radius <= linePosition.x && smallBalls[c].position.x + smallBalls[c].radius >= linePosition.x) && (smallBalls[c].position.y + smallBalls[c].radius >= shoot[i].position.y))
                        {
                            shoot[i].active = false;
                            shoot[i].lifeSpawn = 0;
            ...
            if (meteorsDestroyed == (MAX_BIG_BALLS + MAX_BIG_BALLS * 2 + MAX_BIG_BALLS * 4))
                victory = true;
        }
    }
    else
...
void DrawGame(void)
{
    ...
    if (!gameOver)
    {
        ...
        {
            if (bigBalls[i].active)
                DrawCircleV(bigBalls[i].position, bigBalls[i].radius, DARKGRAY);
            else
                DrawCircleV(bigBalls[i].position, bigBalls[i].radius, Fade(LIGHTGRAY, 0.3f));
        }
        ...
        {
            if (mediumBalls[i].active)
                DrawCircleV(mediumBalls[i].position, mediumBalls[i].radius, GRAY);
            else
                DrawCircleV(mediumBalls[i].position, mediumBalls[i].radius, Fade(LIGHTGRAY, 0.3f));
        }
        ...
        {
            if (smallBalls[i].active)
                DrawCircleV(smallBalls[i].position, smallBalls[i].radius, GRAY);
            else
                DrawCircleV(smallBalls[i].position, smallBalls[i].radius, Fade(LIGHTGRAY, 0.3f));
        }
        ...
        if (pause)
            DrawText("GAME PAUSED", screenWidth / 2 - MeasureText("GAME PAUSED", 40) / 2, screenHeight / 2 - 40, 40, LIGHTGRAY);
    }
    else
        DrawText("PRESS [ENTER] TO PLAY AGAIN", GetScreenWidth() / 2 - MeasureText("PRESS [ENTER] TO PLAY AGAIN", 20) / 2, GetScreenHeight() / 2 - 50, 20, LIGHTGRAY);
